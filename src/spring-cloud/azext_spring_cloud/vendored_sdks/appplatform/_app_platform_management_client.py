...
class AppPlatformManagementClient(MultiApiClientMixin, SDKClient):
    ...
    def configuration_services(self):
        """Instance depends on the API version:
           ...
           * 2021-03-01-preview: :class:`ConfigurationServicesOperations<azure.mgmt.appplatform.v2021_03_01_preview.operations.ConfigurationServicesOperations>`
        """
        api_version = self._get_api_version('configuration_services')
        if api_version == '2021-03-01-preview':
            from .v2021_03_01_preview.operations import ConfigurationServicesOperations as OperationClass
        else:
            raise NotImplementedError("APIVersion {} is not available".format(api_version))
        return OperationClass(self._client, self.config, Serializer(self._models_dict(api_version)), Deserializer(self._models_dict(api_version)))
