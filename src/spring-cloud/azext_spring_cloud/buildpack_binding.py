...
from .custom import (DEFAULT_BUILD_SERVICE_NAME)
...
def buildpack_binding_show(cmd, client, resource_group, service, name, builder_name=None):
    return client.buildpack_binding.get(resource_group,
                                         service,
                                         DEFAULT_BUILD_SERVICE_NAME,
                                         builder_name,
                                         name)
...
def buildpack_binding_list(cmd, client, resource_group, service, builder_name=None):
    return client.buildpack_binding.list(resource_group,
                                          service,
                                          DEFAULT_BUILD_SERVICE_NAME,
                                          builder_name)
...
def create_default_buildpack_binding_for_application_insights(cmd, client, resource_group, name, location,
    ...
    if not disable_app_insights:
        ...
        if binding_resource.properties:
            return client.buildpack_binding.begin_create_or_update(resource_group, name,
                                                                   DEFAULT_BUILD_SERVICE_NAME,
